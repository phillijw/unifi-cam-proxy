    ...
    async def start_video_stream(
        ...
        if not has_spawned or is_dead:
            ...
            cmd = (
                ...
                f" -metadata audioBandwidth=64000.0 "
                f" -metadata audioChannels=1.0 "
                f" -metadata audioFrequency=48000.0 "
                f" -metadata channelId=0.0 "
                f" -metadata extendedFormat=1 "
                f" -metadata hasAudio=1 "
                f" -metadata hasVideo=1 "
                f" -metadata streamId=0.0 "
                f" -metadata videoBandwidth=10000000.0 "
                f" -metadata videoFps=15.0 "
                f" -metadata videoHeight=1920.0 "
                f" -metadata videoWidth=1080.0 "
                f" -b:v 64k -r 15 -f flv "
                f" - | {sys.executable} -m"
                " unifi.clock_sync"
